        ...
        for filename in filenames:
            if filename.endswith(supported_extensions):
                ...
                if doc_entries_in_file:
                    ...
                    output_filename_doc = output_basename + '.rst'
                    output_filepath_doc = os.path.join(output_rst_dir, output_filename_doc)

                    output_filename_src = output_basename + '_src.rst'
                    output_filepath_src = os.path.join(output_rst_dir, output_filename_src)
                    ...
                    with open(output_filepath_doc, 'w', encoding='utf-8') as f:
                        if main_entities:
                    ...
                    print(f"Generated: {output_filepath_doc} with {len(doc_entries_in_file)} entries.")

                    language_map = {
                        '.ah': 'cpp', '.hh': 'cpp', '.cc': 'cpp'
                    }
                    language = language_map.get(os.path.splitext(filepath)[1], 'text') # Default zu 'text'

                    with open(output_filepath_src, 'w', encoding='utf-8') as f:
                    ...
                    print(f"Generated source file: {output_filepath_src}")
